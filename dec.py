...
# Functions from encryption part
...
def reverse_substitute_characters(ciphertext, key):
    substitution_dict = {}
    for letter in range(ord('A'), ord('Z') + 1):
        ascii_value = ord('A') + ((ord(letter) - ord('!') - key) % 26)
        substitution_dict[chr(ascii_value)] = chr(letter)

    plaintext = ""
    for char in ciphertext:
        if char.isalpha() and char.isupper():
            plaintext += substitution_dict.get(char.upper(), char)
        else:
            plaintext += char
    return plaintext
...
def main():
    ...
    decrypted_plaintext = reverse_substitute_characters(ciphertext, key)
